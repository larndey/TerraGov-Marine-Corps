...
/obj/machinery/air_alarm/Initialize(mapload, direction, building = FALSE)
	. = ..()
...
/obj/machinery/air_alarm/proc/handle_heating_cooling()
	return

/obj/machinery/air_alarm/proc/overall_danger_level(turf/T)
...
/obj/machinery/air_alarm/can_interact(mob/user)
	. = ..()
	if(!.)
		return FALSE
...
/obj/machinery/air_alarm/attackby(obj/item/I, mob/user, params)
	. = ..()
...
/obj/machinery/air_alarm/examine(mob/user)
	. = ..()
	if (buildstage < 2)
		. += "It is not wired."
	if (buildstage < 1)
...
/obj/machinery/air_alarm/server
	req_one_access = list(ACCESS_CIVILIAN_ENGINEERING)
	target_temperature = 90

/obj/machinery/air_alarm/server/first_run()
	alarm_area = get_area(src)
	area_uid = alarm_area.uid
	if (name == "alarm")
		name = "[alarm_area.name] Air Alarm"
...
/obj/machinery/air_alarm/proc/reset(wire)
	switch(wire)
		if(WIRE_POWER)
